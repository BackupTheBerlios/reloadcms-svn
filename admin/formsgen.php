...
<tr>
  <th>' . $lang['general']['result'] . '</td>
</tr>
<tr class="input-form-tr">
  <td valign="middle" align="left" class="row1">' . $mesg . '</td>
</tr>
</table>';
}
...
class InputForm
    {
        ...
        function InputForm ($action="", $method="get", $submit="Submit", $target="", $enctype="", $name="", $events="")
        {
            $this->_action = $action;
            $this->_method=$method;
            $this->_target=$target;
        ...
        function show()
        {
            echo "<form style=\"display: inline;\" action=\"$this->_action\" method=\"$this->_method\" name=\"$this->_name\"";
            ...
            if(is_array($this->_rows)) foreach($this->_rows as $key=>$row){
                if ( !empty($row["break"]) ){
                    ...
                    if(isset($this->_help[$key])){
                        $title = $title . '<br>' . $this->_help[$key];
                    }
                    echo "<tr>\n";
                    echo "  <th colspan=\"2\">$title</td>\n";
                    echo "</tr>\n";
                    ...
                    if(isset($this->_help[$key])){
                        $title = $title . '<br><br>' . $this->_help[$key];
                    }

                    echo "<tr class=\"input-form-tr\">\n";
                    echo "  <td valign=\"$row[title_valign]\" align=\"$row[title_align]\" class=\"row1\" $colspan>$title</td>\n";
                    if ( !$colspan ){
                        echo "  <td valign=\"$row[content_valign]\" align=\"$row[content_align]\" class=\"row2\">$row[contents]</td>\n";
                    }
                    echo "</tr>\n";
                }
            }
            echo "<tr class=\"input-form-tr\">\n";
            echo "  <td class=\"input-form-td-break\" align=\"center\" colspan=\"2\"><input type=\"submit\" value=\"$this->_submit\" class=\"input-form-submit\">";
            if(!empty($this->_reset)) echo "<input type=\"reset\" value=\"{$this->_reset}\">";
            echo "</td>\n";
            echo "</tr>\n";
